...
def neighbour_nodes_generate(s, current_node, TreeConn):
    """ This function returns the neighbour nodes of current node(new generated node) 
    within radius of 1 meter"""
    neigh_radius = 1
    neighbour_nodes = []
    for k in range(s): 
...
def best_parent_generate(s, current_node, TreeConn, parent, neighbour_nodes):
    parent_index = len(parent) -1
    current_parent = parent[parent_index]
    dist_current_parent_from_o = np.sqrt((TreeConn[current_parent][0] - TreeConn[0][0])**2 + ((TreeConn[current_parent][1] - TreeConn[0][1])**2))  #distance of k node from first node
    dist_new_from_current_parent = np.sqrt((TreeConn[current_parent][0] - current_node[0])**2 + ((TreeConn[current_parent][1] - current_node[1])**2)) #distance of new node (current) from kth node
    cost_newNode = dist_current_parent_from_o + dist_new_from_current_parent
    for k in neighbour_nodes:
        dist_Kth_from_o = np.sqrt((TreeConn[k][0] - TreeConn[0][0])**2 + ((TreeConn[k][1] - TreeConn[0][1])**2))  #distance of k node from first node
        dist_new_from_k = np.sqrt((TreeConn[k][0] - current_node[0])**2 + ((TreeConn[k][1] - current_node[1])**2)) #distance of new node (current) from kth node
        cost = dist_Kth_from_o + dist_new_from_k
        if cost < cost_newNode:
            cost_newNode_zero = cost
            parent[parent_index] = k

    return parent[parent_index] # returning best parent


def rrt_star(start_node, goal_node, all_obstacles):
    parent = []
    map_x =10
    map_y = 10
    max_distance = 0.5
    ...
    while ~goal_found:
        ...
        distanceCheck = []
        for k in range(s):
            dist = np.sqrt((TreeConn[k][0] - new_rand_node[0])**2 + ((TreeConn[k][1] - new_rand_node[1])**2)) # second norm (euclidian distance)
        ...
        neighbour_nodes = neighbour_nodes_generate(s, current_node, TreeConn)



        if check_collision(current_node[0], current_node[1], all_obstacles):
            plt.plot([nearst_node[0], current_node[0]], [nearst_node[1], current_node[1]], 'y--')
            ...
            bp = best_parent_generate(s, current_node, TreeConn, parent, neighbour_nodes)
            
            ### Plot new edge with best parent
            best_parent_node = TreeConn[bp]
            plt.plot([best_parent_node[0], current_node[0]], [best_parent_node[1], current_node[1]], 'b')
            plt.pause(0.1)
